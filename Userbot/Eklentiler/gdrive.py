...
@Client.on_message(
    filters.command(['gortaklar'], ['!', '.', '/']) & filters.me)
...
async def gdisk(client, message):
    ...
    if message.command[1] in drive_id:
        ...
        os.environ["ORTAK_DRIVE_ADI"] = drive_adi[drive_id.index(
            os.environ["ORTAK_DRIVE_ID"])]
    ...
    await ilk_mesaj.edit(
        f"`{ORTAK_DRIVE_ADI}` __varsayılan olarak ayarlandı..__")
...
async def gara(client, message):
    ...
    try:
        await ilk_mesaj.edit(await
                             ara_drive(arama_kelimesi=message.command[1]),
                             disable_web_page_preview=True)
    except (MessageTooLong, OSError) as hata:
        await hata_log(hata)
        await ilk_mesaj.edit(
            '__Çıktı çok uzun kanka daha spesifik şekilde aramalısın..__')
        return
