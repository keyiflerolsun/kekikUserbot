...
def nobetci_eczane(il=None, ilce=None, cikti='gorsel_veri'):
    ...
    for bak in bugun.findAll('tr')[1:]:
        ...
        mah = None if bak.find('div', class_='my-2') == None else bak.find(
            'div', class_='my-2').text
        adres = bak.find('span', class_='text-capitalize').text
        tarif = None if bak.find(
            'span', class_='text-secondary font-italic') == None else bak.find(
                'span', class_='text-secondary font-italic').text
