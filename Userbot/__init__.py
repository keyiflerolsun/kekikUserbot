...
if (bellenim_surumu.split('-')[-1] !=
        'aws') and (not os.path.exists("ayar.env")):  # Heroku Geçmek için aws
...
if not os.path.isdir(INDIRME_ALANI):
    os.makedirs(INDIRME_ALANI)
...
try:
    kekikUserbot = Client(STRING_SESSION,
                          api_id=int(API_ID),
                          api_hash=API_HASH,
